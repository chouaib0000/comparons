...
const Chatbot: React.FC = () => {
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const welcomeMessage: Message = {
        ...
        text: "Bonjour ! 👋 Je suis votre assistant virtuel Comparons. Je peux vous aider avec toutes vos questions d'assurance. Que souhaitez-vous savoir ?",
        isBot: true,
        timestamp: new Date(),
  ...
  const getBotResponse = (userMessage: string, context: string[]): ChatbotResponse => {
    ...
    if (message.match(/^(bonjour|salut|hello|hi|bonsoir|hey|coucou)$/i)) {
      return {
        text: "Bonjour ! 😊 Ravi de vous parler ! Je suis spécialisé dans les assurances chez Comparons et je connais parfaitement nos offres. Comment puis-je vous accompagner aujourd'hui ?",
    ...
    if (message.match(/(auto|voiture|moto|véhicule|conducteur|permis|malus|bonus|accident)/i)) {
      if (message.includes('malus') || message.includes('malussé')) {
        return {
          text: "🚗 Pas de souci ! Chez Comparons, nous sommes spécialistes des profils malussés. Nous avons des solutions adaptées même avec un coefficient de malus élevé. Nos partenaires acceptent les conducteurs avec historique difficile et nous négocions les meilleurs tarifs pour vous.",
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-96 h-[600px] bg-white rounded-2xl shadow-3xl border border-gray-200 flex flex-col overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white p-4 flex items-center">
            ...
            <div>
              <h3 className="font-bold">Assistant Comparons</h3>
